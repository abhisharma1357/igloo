...
interface IIglooStrategy {
    // Deposit amount of tokens for user
    function deposit(address user, uint256 tokenAmount, uint256 shareAmount) external;
    // Transfer tokens from strategy to user
    function withdraw(address user, uint256 tokenAmount, uint256 shareAmount) external;
...
contract IglooStrategyBase is IIglooStrategy, Ownable {
    ...
    function deposit(address, uint256, uint256) external virtual override onlyOwner {
    }

    function withdraw(address user, uint256 tokenAmount, uint256) external virtual override onlyOwner {
        if (tokenAmount > 0) {
            depositToken.safeTransfer(user, tokenAmount);
        }
    }
...
contract IglooStrategyForPangolinStaking is IglooStrategyBase {
    ...
    */
    ...
    function deposit(address user, uint256 tokenAmount, uint256 shareAmount) external override onlyOwner {
        _claimRewards();
        _harvest(user);
        if (tokenAmount > 0) {
        ...
        if (shareAmount > 0) {
            iglooStrategyStorage.increaseRewardDebt(user, shareAmount);
        }
    }

    function withdraw(address user, uint256 tokenAmount, uint256 shareAmount) external override onlyOwner {
        _claimRewards();
        _harvest(user);
        if (tokenAmount > 0) {
        ...
        if (shareAmount > 0) {
            iglooStrategyStorage.decreaseRewardDebt(user, shareAmount);
        }
    }
    ...
    function _harvest(address user) internal {
        ...
        uint256 userRewardDebt = iglooStrategyStorage.rewardDebt(user);
        uint256 userPendingRewards = (totalRewards >= userRewardDebt) ?  (totalRewards - userRewardDebt) : 0;
        iglooStrategyStorage.setRewardDebt(user, userShares);
        if(userPendingRewards > 0) {
            _safeRewardTokenTransfer(user, userPendingRewards);
        }
        emit Harvest(user, userPendingRewards);
    }
...
contract IglooMaster is Ownable {
    ...
    function deposit(uint256 pid, uint256 amount, address to) external onlyApprovedContractOrEOA {
        ...
        if (amount > 0) {
            ...
            pool.strategy.deposit(to, amount, newShares);
    ...
    function withdraw(uint256 pid, uint256 amountShares, address to) external onlyApprovedContractOrEOA {
        ...
        if (amountShares > 0) {
            ...
            if (pool.withdrawFeeBP > 0 && pool.totalShares > amountShares) {
                ...
                pool.strategy.withdraw(to, lpToSend, amountShares);
                if(nestSplitBips > 0) {
                    ...
                    pool.strategy.withdraw(nestAllocatorAddress, amountToNest, 0);
                ...
                //tell strategy to withdraw lpTokens, send to 'to', and process update
                pool.strategy.withdraw(to, lpFromShares, amountShares);
            }
    ...
    function harvest(uint256 pid, address to) external onlyApprovedContractOrEOA {
        ...
        //call strategy to update for 'to'
        pool.strategy.withdraw(to, 0, 0);
    ...
    function setNestSplitBips(uint256 _nestSplitBips) external onlyOwner {
        require(_nestSplitBips <= MAX_BIPS, "input too high");
    ...
    function setDefaultIpefiDistributionBips(uint256 _defaultIpefiDistributionBips) external onlyOwner {
        require(_defaultIpefiDistributionBips <= MAX_BIPS, "input too high");
